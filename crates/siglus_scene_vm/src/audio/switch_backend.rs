//! Horizon audio sink for the existing Kira mixer.
//!
//! Kira continues to decode, schedule and mix every existing Siglus sound.
//! libnx's audren shell pulls stereo PCM from this backend through the C ABI.
...
impl Backend for SwitchBackend {
    ...
    fn start(&mut self, renderer: Renderer) -> Result<(), Self::Error> {
        let mut slot = renderer_slot().lock().map_err(|_| "audio renderer mutex poisoned")?;
        *slot = Some(renderer);
        Ok(())
    }
}

/// Fill interleaved signed 16-bit stereo samples for libnx audren.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn siglus_switch_audio_render_i16(dst: *mut i16, frames: usize) {
    ...
    output.fill(0);
    let Ok(mut slot) = renderer_slot().lock() else { return; };
    let Some(renderer) = slot.as_mut() else { return; };
