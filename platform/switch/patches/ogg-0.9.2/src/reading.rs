...
impl PageParser {
    ...
    /// You should allocate and fill such an array, in order to pass it to the `parse_segments`
    /// function.
    pub fn new_with_parse_opts(
        header_buf: [u8; 27],
        parse_opts: impl Into<Arc<PageParsingOptions>>,
    ) -> Result<(PageParser, usize), OggReadError> {
...
pub struct BasePacketReader {
    ...
    page_infos: BTreeMap<u32, PageInfo>,
...
impl<T: io::Read + io::Seek> PacketReader<T> {
    ...
    pub fn new_with_page_parse_opts(
        rdr: T,
        pg_parse_opts: impl Into<Arc<PageParsingOptions>>,
    ) -> PacketReader<T> {
        PacketReader {
            rdr,
            pg_parse_opts: pg_parse_opts.into(),
    ...
    fn read_ogg_page(&mut self) -> Result<Option<OggPage>, OggReadError> {
        ...
        let (mut pg_prs, page_segments) = tri!(PageParser::new_with_parse_opts(
            header_buf,
            Arc::clone(&self.pg_parse_opts)
        ));
...
pub mod async_api {
            ...
            loop {
                ...
                self.state = match ::std::mem::replace(&mut self.state, InUpdate) {
                    Head => {
                        ...
                        let tup = tri!(PageParser::new_with_parse_opts(
                            hdr_buf,
                            Arc::clone(&self.parse_opts)
                        ));
    ...
    */
    ...
    impl<T: FuturesAsyncRead> PacketReader<Compat<T>> {
        ...
        /// from other runtimes, and implementing a Tokio `AsyncRead`
        /// compatibility layer oneself is not desired.
        pub fn new_compat_with_page_parse_opts(
            inner: T,
            pg_parse_opts: impl Into<Arc<PageParsingOptions>>,
        ) -> Self {
